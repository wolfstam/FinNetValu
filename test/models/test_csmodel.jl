...
csmodel = FinNetValu.CSModel(Π, C, B, S, ADV, σ, 1., 1., λ_max)
a = FinNetValu.init_a(csmodel, Θ, ϵ)

csmodel2 = FinNetValu.CSModel(Π, [1, -1], B, S, ADV, σ, 1, 1, λ_max)
a2 = FinNetValu.init_a(csmodel2, Θ, ϵ)

csmodel3 = FinNetValu.CSModel(Π, [100, -1], B, S, ADV, σ, 1, 1, λ_max)
a3 = FinNetValu.init_a(csmodel3, Θ, ϵ)

csmodel4 = FinNetValu.CSModel(Π2, C2, B2, S2, ADV2, σ2, 1, 1,
                                λ_max2)
a4 = FinNetValu.init_a(csmodel4, Θ2, [0., 0.])

csmodel5 = FinNetValu.CSModel(Π2, C2, B2, S2, ADV2, σ2, 1, 1,
                                λ_max2)
a5 = FinNetValu.init_a(csmodel5, Θ2, [0.4, 0.4])

csmodel6 = FinNetValu.CSModel(Π2, C2, B2, S2, ADV2, σ2, 1, 1, λ_max2)
a6 = FinNetValu.init_a(csmodel6, Θ2, ϵ2)

csmodel7 = FinNetValu.CSModel(hcat([0; 70]), C2, B2, S2, ADV2,
                                σ2, 1, 1, λ_max2)
a7 = FinNetValu.init_a(csmodel7, Θ2, [0.37, 0.])

csmodel8 = FinNetValu.CSModel(Π2, C2, [0.5], [1.], [50.], σ2,
                                0.4, 20., 33.3; λ_target=0.95*33.3, α=0.5)
a8 = FinNetValu.init_a(csmodel8, Θ2, [0.2, 0.])

csmodel8_2 = FinNetValu.CSModel(Π2_2, C2, [0.5], S2_2, [50.], σ2,
                                0.4, 20., 33.3; λ_target=0.95*33.3, α=0.5)
a8_2 = FinNetValu.init_a(csmodel8_2, Θ2, [0.2, 0.])
...
y = deepcopy(x)
x2 = FinNetValu.init(csmodel2, a2)
x3 = FinNetValu.init(csmodel3, a3)
...
a8_4 = FinNetValu.init_a(csmodel8_4, Θ2, [0.2, 0.])
...
@testset "csmodel" begin
    ...
    @test_throws ArgumentError FinNetValu.CSModel(Π', C, B, S, ADV,
                                                        σ, 1, 1, λ_max)
    @test_throws ArgumentError FinNetValu.CSModel(Π, C, rand(1:4, size(Π, 2)+1),
                                                        S, ADV, σ, 1, 1, λ_max)
    @test_throws ArgumentError FinNetValu.CSModel(Π, C, B,
                                                        rand(1:4, size(Π, 2)+1),
                                                        ADV, σ, 1, 1, λ_max)
    @test_throws ArgumentError FinNetValu.CSModel(Π, C, B, S,
                                                        [100], σ, 1, 1, λ_max)
    @test_throws ArgumentError FinNetValu.CSModel(Π, C, B, S, ADV,
                                                        [0.02, 0.02], 1, 1, λ_max)
    @test_throws ArgumentError FinNetValu.CSModel(Π, C, B, S, ADV,
...
@testset "init_a" begin
    @test FinNetValu.init_a(csmodel, Θ, ϵ) != false
    @test_throws ArgumentError FinNetValu.init_a(csmodel, Θ', ϵ)
    @test_throws ArgumentError FinNetValu.init_a(csmodel, Θ, [1., 1.])

    @test FinNetValu.init_a(csmodel, Θ, ϵ)[end] == 4

    @test FinNetValu.init_a(csmodel, Θ, ϵ)[1:2] == [168., 524.]
    @test FinNetValu.init_a(csmodel8, Θ2, [0.2, 0.])[1:2] == [8., 20.]

    @test FinNetValu.init_a(csmodel, Θ, ϵ) == vcat([sum(Θ[i,:]) - sum(ϵ.*Θ[i,:]) for i in 1:size(Θ,1)],
                                                vec(Θ), ϵ, 4)
end

@testset "numillassets" begin
    @test FinNetValu.numillassets(csmodel, a) == 4
end

@testset "I_ϵview" begin
    @test FinNetValu.I_ϵview(csmodel, a) == [168., 524.]
    @test FinNetValu.I_ϵview(csmodel8, a8) == [8., 20.]
end

@testset "Θview" begin
    @test FinNetValu.Θview(csmodel, a) != false
    @test FinNetValu.Θview(csmodel, a) == Θ
    @test FinNetValu.Θview(csmodel8, a8) == Θ2
end

@testset "ϵview" begin
    @test FinNetValu.ϵview(csmodel, a) != false
    @test FinNetValu.ϵview(csmodel, a) == ϵ
    @test FinNetValu.ϵview(csmodel8, a8) == [0.2, 0.]
end

@testset "init" begin
    @test FinNetValu.init(csmodel6, a6) != false

    C_ϵ = [max(C2[i] - sum(ϵ2.*Θ2[i,:]), 0) for i in 1:size(C2, 1)]
    @test FinNetValu.init(csmodel6, a6) == vcat(vec(hcat(C_ϵ, csmodel6.Π)), csmodel6.S)

    @test FinNetValu.Cview(csmodel2, x2) == [0, 0]
    @test FinNetValu.Cview(csmodel3, x3) == [78, 0]
    @test FinNetValu.Cview(csmodel8, x8) == [2., 4.5]
end

@testset "Piview" begin
    @test FinNetValu.Πview(csmodel, x) == csmodel.Π
    @test FinNetValu.Πview(csmodel6, x6) == csmodel6.Π
end

@testset "Cview" begin
    @test FinNetValu.Cview(csmodel, x) == [78., 144.]
    @test FinNetValu.Cview(csmodel6, x6) == [2., 4.5]
end

@testset "Sview" begin
    @test FinNetValu.Sview(csmodel, x) == csmodel.S
    @test FinNetValu.Sview(csmodel6, x6) == csmodel6.S
end

@testset "leverageratio" begin
    @test FinNetValu.leverageratio(csmodel, x, a) != false
    @test FinNetValu.leverageratio(csmodel8, x8, a8) == [49, 20]
    @test round.(FinNetValu.leverageratio(csmodel8_3, x8_3, a8_3), digits=2) == [43.63, 22.59]
end

@testset "deleverageproportion" begin
    @test FinNetValu.delevprop(csmodel, x, a) != false
    @test FinNetValu.delevprop(csmodel4, x4, a4) == zeros(2)
    @test FinNetValu.delevprop(csmodel6, x6, a6) == [22/90, 0.]
    @test FinNetValu.delevprop(csmodel7, x7, a7)[1] == 0.
...
@testset "compS" begin
    ...
    @test FinNetValu.compS(deepcopy(csmodel4), x4, [0.]) == csmodel4.S
